...
const Landing = {
  ...
  bindEvents() {
    ...
    // Social handles click
    document.querySelectorAll('.social-handle-pill').forEach(pill => {
      pill.addEventListener('click', (e) => {
        const handle = pill.querySelector('strong') ? pill.querySelector('strong').textContent : 'THE CEO';
        const href = pill.getAttribute('href');
        if (href && href !== 'javascript:void(0)') {
          Toast.info(`Opening official channel for ${handle}...`);
        }
      });
    });
  },
  ...
  updateSimulator(v3pt, vDunk, vHandle) {
    ...
    if (previewBadgesEl) {
      ...
      previewBadgesEl.innerHTML = badges.map(b => BadgeHelper.renderTierPill(b.name, b.tier)).join(' ');
    }
  }
};
