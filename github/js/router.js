...
const Router = {
  routes: ['#landing', '#tools', '#membership', '#dashboard'],
  ...
  init() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (anchor) {
        ...
        if (href === '#tools' || href === '#intel-tools') {
          e.preventDefault();
  ...
  handleRoute() {
    ...
    if (hash === '#tools' || hash === '#intel-tools') {
      ...
      if (window.VaultState) VaultState.setActiveTab('builds');
      hash = '#dashboard';
    } else if (hash === '#jumpshots') {
      if (window.VaultState) VaultState.setActiveTab('jumpshots');
      hash = '#dashboard';
    } else if (hash === '#intel') {
      if (window.VaultState) VaultState.setActiveTab('intel');
      hash = '#dashboard';
    } else if (hash === '#videos') {
      if (window.VaultState) VaultState.setActiveTab('videos');
      hash = '#dashboard';
    } else if (hash === '#settings') {
      if (window.VaultState) VaultState.setActiveTab('settings');
      hash = '#dashboard';
    ...
    if (hash === '#dashboard') {
      const state = window.VaultState ? VaultState.getState() : {};
      if (!state.isAuthenticated) {
        Modal.open('modal-auth');
        Toast.warning('Please sign in or continue as guest to access Member Dashboard.');
    ...
    document.querySelectorAll('.nav-link, .drawer-link').forEach(link => {
      ...
      if (targetSection && (href === '#tools' || href === '#intel-tools')) {
    ...
    if (hash === '#dashboard' && window.Dashboard) {
      Dashboard.render();
    }
  },
