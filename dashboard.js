...
(function() {
  const Dashboard = {
    ...
    switchTab(tabName) {
      if (window.VaultState && typeof VaultState.setActiveTab === 'function') {
    ...
    render() {
      ...
      this.switchTab(stateObj.activeTab || 'builds');
    ...
    renderAdminSubTabs(activeSubTab) {
      ...
      panes.forEach(p => {
        p.classList.toggle('active', p.id === `admin-pane-${activeSubTab}`);
      });
    },
